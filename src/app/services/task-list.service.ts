...
export class TaskListService {
  ...
  initializeTaskLists(prj: Project): Observable<Project> {
    ...
    return merge(
      this.add({ id: undefined, name: 'Ready', projectId: id, order: 1 }),
      this.add({ id: undefined, name: 'Processing', projectId: id, order: 2 }),
      this.add({ id: undefined, name: 'Completed', projectId: id, order: 3 })
    ).pipe(
      reduce((r: TaskList[], x: TaskList) => [...r, x], []),
      map((tls: TaskList[]) => ({
        ...prj,
